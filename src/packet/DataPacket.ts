...
function parseRecord(cursor: BufferCursor): Record {
  ...
  switch (val.type) {
    case NAME_TO_QTYPE.A:
      return A.parse(val as Address_Record, cursor);
    case NAME_TO_QTYPE.AAAA:
      return AAAA.parse(val as Address_Record, cursor);
    case NAME_TO_QTYPE.NS:
      return NS.parse(val as DataString_Record, cursor);
    case NAME_TO_QTYPE.CNAME:
      return CNAME.parse(val as DataString_Record, cursor);
    case NAME_TO_QTYPE.PTR:
      return PTR.parse(val as DataString_Record, cursor);
    case NAME_TO_QTYPE.SPF:
    case NAME_TO_QTYPE.TXT:
      return TXT.parse(val as TXT_Record, cursor, len);
    case NAME_TO_QTYPE.MX:
      return MX.parse(val as MX_Record, cursor);
    case NAME_TO_QTYPE.SRV:
      return SRV.parse(val as SRV_Record, cursor);
    case NAME_TO_QTYPE.SOA:
      return SOA.parse(val as SOA_Record, cursor);
    case NAME_TO_QTYPE.NAPTR:
      return NAPTR.parse(val as NAPTR_Record, cursor);
    default:
      return CNAME.parse(val as DataString_Record, cursor);
  }
}
